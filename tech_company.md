...
> **Bemærkning:** Jeg har skrevet mine queries i MySQL. Svarene står under hvert spørgsmål. Et par steder var jeg lidt i tvivl om `WHERE` versus `HAVING`, og om hvordan NULL opfører sig i joins, men jeg fik det til at give mening til sidst. Jeg håber denne er fyldesgørende og lever op til kravene. 
...
Det giver mig to navne: JONES og JAMES.
...
KING har department_number som NULL, så han havner i sin egen gruppe.
Department 40 (OPERATIONS) har ingen ansatte og dukker slet ikke op her,
fordi jeg kun tæller rækker i employees. Det kommer igen i JOIN-opgaverne.
...
Commission er NULL for de fleste. Hvis jeg bare skriver SUM(salary + commission),
ryger de rækker ud hvor commission er NULL. Derfor pakker jeg commission ind i
...
department_name til NULL når der ikke er noget match.
...
ansatte. OPERATIONS kommer nu med og får COUNT som 0, fordi COUNT(employee_number)
ikke tæller NULL-værdier med.
...
"manager". For hver medarbejder slår jeg manager_id op mod employee_number i den anden
kopi af tabellen, så jeg får medarbejderen ved siden af den person der er deres
...
Jeg bruger LEFT JOIN i stedet for INNER, så KING også kommer med (med manager som
NULL). Vil jeg kun have dem der faktisk har en chef, kan jeg bruge INNER JOIN.
...
HAVING filtrerer EFTER GROUP BY, altså på de grupperede rækker, mens WHERE filtrerer
...
Jeg starter i employees og hopper via join-tabellen employees_leaders over i leaders.
